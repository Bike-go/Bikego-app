...
def send_registration_email(email, token):
    ...
    try:
        template_path = os.path.join(
            current_app.root_path, "static", "html", "registration.html"
        )
...
def send_reset_email(email, email_new, token):
    ...
    try:
        template_path_verification = os.path.join(
            current_app.root_path, "static", "html", "verification.html"
        )
        template_path_info = os.path.join(
            current_app.root_path, "static", "html", "emailInfo.html"
        )
...
def send_password_change(email, token):
    ...
    try:
        template_path = os.path.join(
            current_app.root_path, "static", "html", "passwordChange.html"
        )
        with open(template_path, "r", encoding="utf-8") as file:
            body = file.read()
        body = body.replace("{{URL}}", url)
...
def send_successfully_password_change(email):
    try:
        template_path = os.path.join(
            current_app.root_path, "static", "html", "successfulyPasswordChange.html"
        )
        with open(template_path, "r", encoding="utf-8") as file:
            body = file.read()
    except FileNotFoundError:
...
def send_contact_form(email, name, message):
    try:
        template_path = os.path.join(
            current_app.root_path, "static", "html", "contactForm.html"
        )
        template_path_info = os.path.join(
            current_app.root_path, "static", "html", "contactFormInfo.html"
        )
        with open(template_path, "r", encoding="utf-8") as file:
            body = file.read()
        with open(template_path_info, "r", encoding="utf-8") as file:
